...
const emailPlaceholder = computed(() => {
  if (usertype.value === '1') {
    return "Correo electrónico. Ej: matias.calderon@outlook.cl";
  } else if (usertype.value === '2') {
    return "Correo electrónico. Ej: admin@volunteers.cl";
  }
...
const sendForm = () => {
  ...
  if (usertype.value == 2) { 
    axios.post("http://localhost:8080/admin/login", body).then(e => { // hay que cambiar la ruta a la db de administradores correspondiente xD
      if (!e.data.success) {
        window.alert("Correo o contraseña incorrectas!")
        return
      }
